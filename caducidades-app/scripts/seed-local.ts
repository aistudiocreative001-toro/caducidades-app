...
function parseCoste(valor: string): number {
  if (!valor) return 0;
...
function calcularFechaMes(fechaISO: string): string {
...
function calcularDias(fechaISO: string): number {
  const hoy = new Date();
...
function calcularEstado(dias: number, estadoActual: string): string {
  const estadosFijos = ['ROTO', 'VENDIDO', 'VENDIDO CADUCADO', 'REGALO CADUCADO', 'MOVIDO'];
  if (estadosFijos.includes(estadoActual.toUpperCase())) return estadoActual;
...
for (const row of result.data as Record<string, string>[]) {
  ...
  const fechaISO = parseFecha(fechaStr);
